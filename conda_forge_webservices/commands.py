...
def issue_comment(org_name, repo_name, issue_num, title, comment, comment_id=None):
    ...
    app_repo = github.Github(APP_GH_TOKEN).get_repo("{}/{}".format(org_name, repo_name))
    ...
    if any(command.search(text) for command in send_pr_commands):
        ...
        try:
            ...
            forked_user_gh.create_fork(gh.get_repo("{}/{}".format(org_name, repo_name)))
            # we have to wait since the call above is async
            for i in range(NUM_GH_API_TRIES):
                try:
                    forked_user_repo = gh.get_repo(
                        "{}/{}".format(forked_user, repo_name)
                    )
                    break
                except Exception as e:
                    if i < 4:
                ...
                if user is None:
                    ...
                    if _changed_anything is None:
                        ...
                        if not _changed_anything:
                            err_msg = "the recipe already has maintainer @%s" % user
                            to_close = True
                        else:
                            do_rerender = False
                            check_bump_build = False
...
def restart_pull_request_ci(repo, pr_num):
    ...
    if drone_status:
        ...
        session.post(
            "/api/repos/conda-forge/%s/builds/%s" % (repo.name, drone_build),
        )
